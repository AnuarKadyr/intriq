...
import "./PriceVolumeLoadingAnimation.css";
...
function createCheckSvg(yOffset: number, index: number): SVGElement {
  const checkmarkIdPrefix = "pvLoadingCheckSVG-";
  const checkmarkCircleIdPrefix = "pvLoadingCheckCircleSVG-";
...
interface PriceVolumeLoadingAnimationProps {
  onComplete?: () => void;
}

const PriceVolumeLoadingAnimation = ({ onComplete }: PriceVolumeLoadingAnimationProps) => {
  ...
  return (
    <div className={`pv-loading-page ${isFadingOut ? 'pv-loading-fade-out' : ''}`}>
      <div className="pv-loading-phrase-box">
        <svg width="100%" height="100%">
          <defs>
            ...
            >
              ...
              <rect width="100%" height="100%" fill="url(#pvLinearGradient)" />
            </mask>
          </defs>
          <g width="100%" height="100%" style={{ mask: "url(#pvMask)" }}>
            <g id="pvPhrases" ref={phrasesRef}></g>
          </g>
        </svg>
      </div>
      <div className="pv-loading-footer">
        <img src={logoWhite} alt="Logo" className="pv-loading-logo" />
      </div>
    </div>
  );
};

export default PriceVolumeLoadingAnimation;
